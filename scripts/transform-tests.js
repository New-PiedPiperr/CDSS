  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      (upperLine === 'TEST' ||
        ...
        upperLine === 'TEST:' ||
        upperLine === 'TESTS:' ||
        upperLine.startsWith('SPECIAL TEST') ||
        upperLine.startsWith('CONFIRMATORY TEST'))
    ) {
      inTestSection = true;
      continue;
    }
    ...
    if (
      inCondition &&
      (upperLine.includes('RADIOGRAPHIC') ||
        upperLine.includes('X-RAY') ||
        upperLine.includes('MRI') ||
        upperLine.includes('ULTRASOUND'))
    ) {
      if (line.length > 5) {
...
function parseAllTestsFromRawText(rawText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      !upperLine.includes('OBSERVATION')
    ) {
      if (currentCondition && currentTests.length > 0) {
        conditionTests[currentCondition] = [...currentTests];
      }
    ...
    if (
      inTestSection &&
      (upperLine === 'OBSERVATION' ||
        upperLine === 'OBSERVATIONS' ||
    ...
    if (currentCondition && !inTestSection) {
      if (
        upperLine.startsWith('RADIOGRAPHIC CONFIRMATION') ||
...
/**
 * Check if a test name is invalid (e.g., section headers)
 */
function isInvalidTestName(name) {
  const invalidNames = [
    'test',
    'tests',
    'test:',
  ...
  return invalidNames.includes(normalizedName) || normalizedName.length < 3;
}

/**
 * Clean up and provide default procedure for tests with empty procedures
 */
function cleanupTest(test) {
  // Ensure name is provided
  if (!test.name || isInvalidTestName(test.name)) {
    return null;
  }

  // Provide meaningful procedure for imaging tests with empty procedure
  if (!test.procedure || test.procedure.trim() === '') {
    const lowerName = test.name.toLowerCase();
    if (lowerName.includes('mri')) {
      test.procedure =
        'MRI imaging study to visualize soft tissue structures and confirm diagnosis.';
    } else if (lowerName.includes('x-ray') || lowerName.includes('radiograph')) {
      test.procedure =
        'Radiographic imaging to assess bony structures and joint alignment.';
    } else if (lowerName.includes('ultrasound')) {
      test.procedure = 'Ultrasound imaging to assess soft tissue structures.';
    } else if (lowerName.includes('ct') || lowerName.includes('computed tomography')) {
      test.procedure = 'CT scan to provide detailed cross-sectional imaging.';
    } else if (lowerName.includes('confirmation')) {
      test.procedure = 'Imaging study to confirm clinical diagnosis.';
    } else {
      test.procedure = 'Perform clinical test as described in standard protocols.';
    }
  }
...
function buildTestsArray(condition, allTests) {
  ...
  for (const test of rawTests) {
    const cleanedTest = cleanupTest(test);
    if (!cleanedTest) continue;

    const normalizedName = cleanedTest.name.toLowerCase().trim();
    if (!seenNames.has(normalizedName)) {
      seenNames.add(normalizedName);
      tests.push(cleanedTest);
    }
  }
  ...
  if (condition.observations && condition.observations.length > 0) {
    for (const obs of condition.observations) {
      ...
      if (
        upperObs.includes('TEST') ||
        upperObs.includes("'S ") ||
        obs.includes('The examiner') ||
        obs.includes('Patient') ||
        obs.includes('positive if') ||
        obs.includes('Positive if')
      ) {
        ...
        if (
          ...
          cleanedTest.name !== 'Clinical Observation'
        ) {
          seenNames.add(normalizedName);
          tests.push(cleanedTest);
        }
...
function processRuleFile(jsonPath) {
  ...
  console.log(
    `  Found test sections for ${Object.keys(allTests).length} conditions in raw text`
  );
  ...
  for (const condition of data.conditions) {
    ...
    if (tests.length === 0) {
      tests.push({
        ...
        procedure:
          'Physical examination and clinical assessment based on presenting symptoms',
      });
    }
...
async function transformAllRules() {
  ...
  const jsonFiles = files.filter(
    (f) => f.endsWith('.json') && !f.includes('index') && !f.includes('clinical-tests')
  );
