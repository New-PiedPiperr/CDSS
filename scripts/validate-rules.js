...
function validateFile(filePath) {
  ...
  for (const [id, question] of questionMap) {
    for (const option of question.options || []) {
      ...
      if (isInstructionalText(option.value)) {
        ...
        warnings.push(`Instructional text as option in ${id}: "${option.value.substring(0, 50)}..."`);
      }
  ...
  if (orphans.length > 0) {
    warnings.push(`${orphans.length} unreferenced questions (may be intentional): ${orphans.slice(0, 3).join(', ')}${orphans.length > 3 ? '...' : ''}`);
  }
...
function main() {
  ...
  const files = fs.readdirSync(RULES_DIR).filter(f => f.endsWith('.json') && f !== 'index.json');
